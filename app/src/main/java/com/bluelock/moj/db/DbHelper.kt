...
import com.bluelock.moj.di.FacebookDownloadEntity
...
class DbHelper(context: Context?) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val sqlCreateTableFacebookDownloads =
            "CREATE TABLE " + FacebookDownloadEntity.TABLE_NAME + " (" + FacebookDownloadEntity.COLUMN_DOWNLOAD_ID + " LONG PRIMARY KEY, " + FacebookDownloadEntity.COLUMN_FILE_NAME + " TEXT NOT NULL, " + FacebookDownloadEntity.COLUMN_FILE_URI + " TEXT, " + FacebookDownloadEntity.COLUMN_OUTPUT_PATH + " TEXT, " + FacebookDownloadEntity.COLUMN_IS_WATERMARKED + " BOOLEAN, " + FacebookDownloadEntity.COLUMN_STATE + " INTEGER, " + FacebookDownloadEntity.COLUMN_VIDEO_SOURCE + " INTEGER, " + FacebookDownloadEntity.COLUMN_DOWNLOAD_TIME + " LONG NOT NULL " + ");"
    ...
    fun insertVideo(video: FVideo, count: Int): Boolean {
        ...
        if (count == 10) {
            val query =
                "DELETE FROM " + FacebookDownloadEntity.TABLE_NAME + " WHERE " + FacebookDownloadEntity.COLUMN_DOWNLOAD_TIME + " = (SELECT MIN( " + FacebookDownloadEntity.COLUMN_DOWNLOAD_TIME + ") FROM " + FacebookDownloadEntity.TABLE_NAME + ")"
            db.execSQL(query)
        }
    ...
    fun getVideo(downloadId: Long): FVideo? {
        val db = this.readableDatabase
        val res = db.rawQuery(
            "select * from " + FacebookDownloadEntity.TABLE_NAME + " where " + FacebookDownloadEntity.COLUMN_DOWNLOAD_ID + " = " + downloadId,
            null
        )
    ...
    fun updateState(downloadId: Long, state: Int) {
        val db = this.writableDatabase
        db.execSQL(
            "UPDATE " + FacebookDownloadEntity.TABLE_NAME + " SET " + FacebookDownloadEntity.COLUMN_STATE + " = " + state + " WHERE " + FacebookDownloadEntity.COLUMN_DOWNLOAD_ID + " = " + downloadId
        )
        db.close()
    }
    ...
    fun setUri(downloadId: Long, uri: String) {
        val db = this.writableDatabase
        db.execSQL(
            "UPDATE " + FacebookDownloadEntity.TABLE_NAME + " SET " + FacebookDownloadEntity.COLUMN_FILE_URI + " = '" + uri + "' WHERE " + FacebookDownloadEntity.COLUMN_DOWNLOAD_ID + " = " + downloadId
        )
        db.close()
    }
